 ...
 * /api/employees without full page reloads.
 */
...
async function loadEmployees(page = null) {
  ...
  const data = await shAjax(`${window.API_BASE}/employees?${params.toString()}`);
...
async function loadPositions(departmentId, selectedId = '') {
  ...
  const data = await shAjax(`${window.API_BASE}/search?action=positions_by_department&department_id=${departmentId || ''}`);
...
async function openEditModal(employeeId) {
  const data = await shAjax(`${window.API_BASE}/employees?action=get&employee_id=${employeeId}`);
...
async function handleSaveEmployee(e) {
  ...
  const data = await shAjax(`${window.API_BASE}/employees`, { method: 'POST', body: formData });
...
function deleteEmployee(employeeId, name) {
  shConfirmDelete(name, async () => {
    const data = await shPost(`${window.API_BASE}/employees`, { action: 'delete', employee_id: employeeId });
    if (data && data.success) {
      shToast(data.message, 'success');
      loadEmployees();
    }
...
async function toggleStatus(employeeId, active) {
  const data = await shPost(`${window.API_BASE}/employees`, { action: 'toggle_status', employee_id: employeeId, active });
  if (data && data.success) {
    shToast(data.message, 'success');
    loadEmployees();
  }
