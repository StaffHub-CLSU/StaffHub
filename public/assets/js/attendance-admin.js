...
async function loadAttendance(page = null) {
  ...
  const data = await shAjax(`${window.API_BASE}/attendance?${params.toString()}`);
...
async function verifyOne(attendanceId, btn) {
  btn.disabled = true;
  const data = await shPost(`${window.API_BASE}/attendance`, { action: 'verify', attendance_id: attendanceId });
  if (data) {
...
async function submitVerifyRange() {
  ...
  const data = await shPost(`${window.API_BASE}/attendance`, { action: 'verify_range', date_from: dateFrom, date_to: dateTo });
  if (data) {
    shToast(data.message, data.success ? 'success' : 'danger');
