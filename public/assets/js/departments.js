...
async function loadDepartments() {
  const data = await shAjax(`${window.API_BASE}/departments?action=list_departments`);
...
async function loadPositions() {
  const data = await shAjax(`${window.API_BASE}/departments?action=list_positions`);
  if (!data || !data.success) return;
...
async function saveDepartment(e) {
  ...
  const data = await shAjax(`${window.API_BASE}/departments`, { method: 'POST', body: formData });
  if (!data) return;
...
function deleteDept(id, name) {
  shConfirmDelete(name, async () => {
    const data = await shPost(`${window.API_BASE}/departments`, { action: 'delete_department', department_id: id });
...
function deletePos(id, name) {
  shConfirmDelete(name, async () => {
    const data = await shPost(`${window.API_BASE}/departments`, { action: 'delete_position', position_id: id });
