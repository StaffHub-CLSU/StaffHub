...
async function loadPayrollHistory(page = null) {
  ...
  const data = await shAjax(`${window.API_BASE}/payroll?${params.toString()}`);
...
async function processSinglePayroll() {
  ...
  const data = await shPost(`${window.API_BASE}/payroll`, { action: 'process', employee_id: employeeId, period_start: periodStart, period_end: periodEnd, bonuses, deductions });
  if (data) {
...
async function processBatchPayroll() {
  ...
  const data = await shPost(`${window.API_BASE}/payroll`, { action: 'process_batch', period_start: periodStart, period_end: periodEnd });
  if (data) {
    shToast(data.message, data.success ? 'success' : 'danger');
...
async function viewPayslip(payrollId) {
  const data = await shAjax(`${window.API_BASE}/payroll?action=get&payroll_id=${payrollId}`);
