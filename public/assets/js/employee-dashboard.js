 ...
 * AJAX Clock In / Clock Out actions against /api/attendance.
 */
...
async function doClockAction(action) {
  ...
  const data = await shPost(`${window.API_BASE}/attendance`, { action });
  ...
  const status = await shAjax(`${window.API_BASE}/attendance?action=today_status`);
