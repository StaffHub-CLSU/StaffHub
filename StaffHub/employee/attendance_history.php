...
$result = $attendanceModel->search($filters, $page, 12);
...
<div class="sh-card p-3 mb-3">
  <form method="GET" class="row g-2">
    <div class="col-md-4"><input type="date" class="form-control" name="date_from" value="<?= e($_GET['date_from'] ?? '') ?>" placeholder="From"></div>
    <div class="col-md-4"><input type="date" class="form-control" name="date_to" value="<?= e($_GET['date_to'] ?? '') ?>" placeholder="To"></div>
    <div class="col-md-3">
  ...
  <div class="d-flex justify-content-between align-items-center p-3 border-top">
    ...
    <nav><ul class="pagination pagination-sm mb-0">
      <?php for ($i = 1; $i <= $result['total_pages']; $i++): ?>
        <li class="page-item <?= $i === $page ? 'active' : '' ?>"><a class="page-link" href="?page=<?= $i ?>&date_from=<?= e($_GET['date_from'] ?? '') ?>&date_to=<?= e($_GET['date_to'] ?? '') ?>&status=<?= e($_GET['status'] ?? '') ?>"><?= $i ?></a></li>
      <?php endfor; ?>
    </ul></nav>
  </div>
  <?php endif; ?>
