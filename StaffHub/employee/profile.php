...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($formType === 'update_contact') {
        ...
        if ($employeeModel->emailExists($email, $employeeId)) {
            $validator->addError('email', 'This email is already used by another employee.');
        }

        if ($validator->fails()) {
            $errorMsg = $validator->firstError();
        } else {
            $data = $employee; // reuse existing values for unchanged fields
    ...
    if ($formType === 'change_password') {
        ...
        $validator = new Validator();
        $validator->required($currentPassword, 'current_password', 'Current password')
                  ->required($newPassword, 'new_password', 'New password')
                  ->passwordStrength($newPassword, 'new_password');

        $userRow = $userModel->findById((int) $employee['user_id']);
        if (!User::verifyPassword($currentPassword, $userRow['password'])) {
        ...
        if ($validator->fails()) {
            ...
            $userModel->updatePassword((int) $employee['user_id'], $newPassword);
...
<div class="row g-3">
  ...
  <div class="col-lg-8">
    <div class="sh-card p-4 mb-3">
      ...
      <form method="POST">
        <input type="hidden" name="form_type" value="update_contact">
        <div class="row g-3">
