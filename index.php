...
if (Authentication::isLoggedIn()) {
    header('Location: ' . (Authentication::isAdmin() ? 'admin/dashboard.php' : 'employee/dashboard.php'));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($validator->fails()) {
        ...
        if ($result['success']) {
            header('Location: ' . ($result['role'] === 'admin' ? 'admin/dashboard.php' : 'employee/dashboard.php'));
            exit;
        }
...
(function () {
  ...
  } catch (err) { /* localStorage unavailable — fall back to defaults */ }
})();
</script>
    ...
    <div class="sh-login-side d-none d-lg-flex">
      <div class="sh-login-side-brand">
        <div class="sh-brand-mark sh-logo-icon" aria-label="StaffHub logo">
          <img src="logo wo text.png" alt="StaffHub logo">
        </div>
        <div>
      ...
      <div class="sh-login-card">
        ...
        <form method="POST" novalidate id="loginForm">
          ...
          <div class="sh-login-input-group">
            ...
            <input type="text" name="username" class="form-control <?= isset($fieldErrors['username']) ? 'is-invalid' : '' ?>" id="loginUsername" placeholder="Enter your username" required autofocus aria-invalid="<?= isset($fieldErrors['username']) ? 'true' : 'false' ?>" aria-describedby="loginUsernameError" value="<?= e($_POST['username'] ?? '') ?>">
          </div>
